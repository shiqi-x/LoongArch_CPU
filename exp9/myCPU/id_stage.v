module id_stage(
    ...
    input  wire         inst_is_ldw_exe,
    input  wire  [31:0] forward_exe,
    // mem_to_id
...
wire        inst_is_ldw;
wire        r1_forward_is_exe;
wire        r1_forward_is_mem;
wire        r1_forward_is_wb;
wire        r2_forward_is_exe;
wire        r2_forward_is_mem;
wire        r2_forward_is_wb;
wire [31:0] rj_value;
wire [31:0] rkd_value;
wire [31:0] imm;
wire [31:0] br_offs;
...
assign r1_forward_is_exe = (rj_src == rd_dest_exe) && (|rj_src);
assign r1_forward_is_mem = (rj_src == rd_dest_mem) && (|rj_src);
assign r1_forward_is_wb  = (rj_src == rd_dest_wb ) && (|rj_src);
assign r2_forward_is_exe = (rk_src == rd_dest_exe) && (|rk_src);
assign r2_forward_is_mem = (rk_src == rd_dest_mem) && (|rk_src);
assign r2_forward_is_wb  = (rk_src == rd_dest_wb ) && (|rk_src);
assign rj_value  = r1_forward_is_exe ? forward_exe :
                   r1_forward_is_mem ? forward_mem :
                   r1_forward_is_wb  ? forward_wb  :
                   rf_rdata1;
assign rkd_value = r2_forward_is_exe ? forward_exe :
                   r2_forward_is_mem ? forward_mem :
                   r2_forward_is_wb  ? forward_wb  :
                   rf_rdata2;
assign data_sram_wdata_id = rkd_value;

assign inst_is_ldw      = inst_ld_w;
...
assign rk_src           = {5{~sel_rk_src_zero}} & {5{valid_id}} & rf_raddr2;
// ##########我要骂人了，为什么之前rk_src这里最后写了rf_waddr_id上一个实验过了，第九个实验才发现上一个实验的问题，无语了...
...
assign pc_id = pc;
assign inst_is_ldw_id = inst_is_ldw;

assign block = (r1_forward_is_exe || r2_forward_is_exe) && inst_is_ldw_exe;
//##########任务9中有了前递之后修改了block的内容，这样可以不修改br_taken_cancle的部分
